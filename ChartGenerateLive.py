...
try:
    ...
    print("Connecting to open 'NewData.xlsx' workbook...")
    wb = xw.Book('NewData.xlsx') 

    # Get the sheets you need
    sheet_chart = wb.sheets['ChartData']
    sheet_1 = wb.sheets['Sheet1']

    # Read the 'ChartData' sheet into a DataFrame
    print("Reading 'ChartData' sheet...")
    df_chart = sheet_chart.used_range.options(pd.DataFrame, index=False, header=True).value
    
    # Read the 'Sheet1' sheet into a DataFrame
    print("Reading 'Sheet1' sheet...")
    df_sheet1 = sheet_1.used_range.options(pd.DataFrame, index=False, header=True).value

    # Read the table data (G1:I13)
    # This replaces: pd.read_excel('NewData.xlsx', sheet_name='ChartData', usecols='G:I', nrows=12)
    print("Reading table data...")
    df_table_data = sheet_chart.range('G1:I13').value
    df_table = pd.DataFrame(df_table_data[1:], columns=df_table_data[0])
    print("Data read successfully.")

except Exception as e:
    print(f"ERROR: Could not read data from 'NewData.xlsx'.")
    print(f"Details: {e}")
    print("Please ensure 'NewData.xlsx' is open and 'xlwings' is installed (`pip install xlwings`).")
    sys.exit() # <-- Stop the script if data reading fails
...
# df_sheet1 = pd.read_excel('NewData.xlsx', sheet_name='Sheet1') # <-- No longer needed
...
# Table data
# df_table = pd.read_excel('NewData.xlsx', sheet_name='ChartData', usecols='G:I', nrows=12) # <-- No longer needed
...
<head>
    ...
    <meta http-equiv="refresh" content="60">  <title>Options Data Chart</title>
